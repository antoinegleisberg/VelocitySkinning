...
namespace cgp
{
	...
	numarray<affine_rt> skeleton_animation_structure::evaluate_local(float t) const
	{
		...
		for(size_t kj=0; kj<N_joint; ++kj)
		{
			...
			rotation_transform T_rotation = rotation_transform::lerp(T0.rotation, T1.rotation, alpha);

			vec3 const T_translation = T0.translation * (1.0f - alpha) + T1.translation * alpha;

			affine_rt T = affine_rt(T_rotation, T_translation);
